...
public class DataValidator {
	...
	public static boolean isEmail(String value) {
		...
		if(isNotNull(value)) {
			try {
				return value.matches(emailRegix);
			} catch (NumberFormatException e) {
				return false;
			}
		}else {
	...
	public static boolean isName(String value) {
		String nameRegix = "^[^-\\s][\\p{L} .'-]+$";
		if(isNotNull(value)) {
			try {
				return value.matches(nameRegix);
			} catch (NumberFormatException e) {
				return false;
			}
		}else {
	...
	public static boolean isRollNo(String value) {
		String rollRegix = "[a-zA-Z]{2}[0-9]{3}";
		if(isNotNull(value)) {
			try {
				return value.matches(rollRegix);
			} catch (NumberFormatException e) {
				return false;
			}
		}else {
	...
	public static boolean isPassword(String value) {
		String passRegix = "(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[@#$%^&+=])(?=\\S+$).{8,12}";
		if(isNotNull(value)) {
			try {
				return value.matches(passRegix);
			} catch (NumberFormatException e) {
				return false;
			}
		}else {
